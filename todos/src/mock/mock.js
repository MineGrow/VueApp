...
export default {
    ...
    start() {   // 初始化函数
        ...
        mock.onGet('/todo/list').reply(config => {  // config 指前台传过来的值
            let mockTodo = Todos.map(tode => {  // 重组 Todos 数组，变成我们想要的数据
                return {
                    id: tode.id,
                    title: tode.title,
                    count: tode.record.filter((data) => {
                        ...
                        return false;
                    }).length, // 过滤到record里面，‘checked’为true的数据，因为它们被完成了
                    locked: tode.locked,
                    isDelete: tode.isDelete
                };
            }).filter(tode => {
                if (tode.isDelete === true) return false; //  过滤掉‘isDelete’为true的，因为已经被删除了
                return true;
            });
